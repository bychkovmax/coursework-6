function mail_form ( )
{
	...
	if ( document.subscription_form.email_subscribe.value == "" )
	{
		alert ( "Пожалуйста заполните поле" );
		mail = false;
	}
	else(
		alert('Теперь ждите новостей!')
	)
...
function contact_form ( )
{
	...
	if (( document.message_form.name.value == false ) && ( document.message_form.phone.value == false )  && ( document.message_form.email.value == false )  && ( document.message_form.message.value == false ))
	{
		alert ( "error" );
		message = false;
	}
	else(
		alert('Сообщение отправлено. Спасибо вам!')
	)
